...
public class UserApiTests {
    ...
    public void assertFindAllJsonApiIsParsedCorrectlyAndContains11Elements() {
        List<User> users = this.restTemplate.exchange("/api/user/", HttpMethod.GET, null, new ParameterizedTypeReference<List<User>>() {}).getBody();
        assertThat(users).hasSize(11);
    }

    @Test
    public void verifyFindOneJsonApi() {
        User user = this.restTemplate.getForObject("/api/user/MkHeck", User.class);
